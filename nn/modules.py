...
class Conv2d(nn.Module):
    def __init__(self, in_c, out_c, k_size, stride=1, padding=0, leakyReLU=False):
...
class SpatialPyramidPool2d(nn.Module):
    def __init__(self, in_channels, out_channels, k=(5, 9, 13)):
        ...
        self.cv1 = Conv2d(in_channels, planes, k_size=1, stride=1)
        self.pools = nn.ModuleList(
            [nn.MaxPool2d(kernel_size=x, stride=1, padding=x // 2) for x in k])
        self.cv2 = Conv2d(planes * (len(k) + 1), out_channels, k_size=1, stride=1)
