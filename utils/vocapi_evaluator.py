...
class VOCAPIEvaluator():
    ...
    def __init__(self,
        ...
        # dataset
        self.dataset = VOCDataset(root=data_root,
    ...
    def evaluate(self, net):
        ...
        for i in range(num_images):
            ...
            for j in range(len(self.labelmap)):
                ...
                c_dets = np.hstack((c_bboxes, c_scores[:, np.newaxis])).astype(np.float32,
                                                                               copy=False)
                ...
                for im_ind, index in enumerate(self.dataset.ids):
                    ...
                    for k in range(dets.shape[0]):
                        f.write('{:s} {:.3f} {:.1f} {:.1f} {:.1f} {:.1f}\n'.format(
                            index[1], dets[k, -1], dets[k, 0] + 1, dets[k, 1] + 1, dets[k, 2] + 1,
                            dets[k, 3] + 1))
